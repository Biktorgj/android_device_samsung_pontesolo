/*
 * Copyright (C) 2008 The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */

#include <fcntl.h>
#include <errno.h>
#include <math.h>
#include <poll.h>
#include <unistd.h>
#include <dirent.h>
#include <sys/select.h>
#include <utils/Log.h>

#include "BioHRM.h"

#define LOG_TAG "AccelerometerSensor"

/*****************************************************************************/
BioHRMSensor::BioHRMSensor()
    : SensorBase(NULL, "hrm_lib_sensor"),
      mEnabled(0),
      mInputReader(4),
      mHasPendingEvent(false)
{
    mPendingEvent.version = sizeof(sensors_event_t);
    mPendingEvent.sensor = ID_HRM;
    mPendingEvent.type = SENSOR_TYPE_HEART_RATE;
    memset(mPendingEvent.data, 0, sizeof(mPendingEvent.data));

    if (data_fd) {
        strcpy(input_sysfs_path, "/sys/class/input/");
        strcat(input_sysfs_path, input_name);
        strcat(input_sysfs_path, "/device/");
        input_sysfs_path_len = strlen(input_sysfs_path);
    }
}

BioHRMSensor::~BioHRMSensor() {

    ALOGD("BioHRMSensor::~BioHRMSensor()");
    if (mEnabled) {
        enable(0, 0);
    }
}

int BioHRMSensor::setInitialState()
{
    return 0;
}

int BioHRMSensor::enable(int32_t handle, int en) {
    int err;
    if (en != mEnabled) {
         err = sspEnable(LOG_TAG, SSP_ACCEL, en);
         if(err >= 0){
             mEnabled = en;
             setInitialState();

             return 0;
         }
         return -1;
    }
    return 0;
}


bool BioHRMSensor::hasPendingEvents() const {
    /* FIXME probably here should be returning mEnabled but instead
      mHasPendingEvents. It does not work, so we cheat.*/
    //ALOGD("BioHRMSensor::~hasPendingEvents %d", mHasPendingEvent ? 1 : 0 );
    return mHasPendingEvent;
}


int BioHRMSensor::setDelay(int32_t handle, int64_t ns)
{
    int fd;

    strcpy(&input_sysfs_path[input_sysfs_path_len], "hrm_poll_delay");
    fd = open(input_sysfs_path, O_RDWR);
    if (fd >= 0) {
        char buf[80];
        sprintf(buf, "%lld", ns); // Some flooring to match stock value
        write(fd, buf, strlen(buf)+1);
        close(fd);
        return 0;
    }

    ALOGD("BioHRMSensor: fail to set delay through %s.", input_sysfs_path);
    return -1;
}


int BioHRMSensor::readEvents(sensors_event_t* data, int count)
{
	const float SNR_SIG_FIGS = 10000.0f;
	const int HR_MAX = 300.0f / 1; // raw data unit seems to be 1
    if (count < 1)
        return -EINVAL;

    if (mHasPendingEvent) {
        mHasPendingEvent = false;
        mPendingEvent.timestamp = getTimestamp();
        *data = mPendingEvent;
        return mEnabled ? 1 : 0;
    }

    ssize_t n = mInputReader.fill(data_fd);
    if (n < 0)
        return n;
    int numEventReceived = 0;
    input_event const* event;

    while (count && mInputReader.readEvent(&event)) {
        int type = event->type;
        if (type == EV_REL) {
            float value = event->value;
			switch (event->code){
				case REL_X:
					ALOGE("BioHRM Sensor: %i", event->value);
					mPendingEvent.heart_rate.bpm = value;
					break;
				case REL_Y:
					ALOGE("BioHRM Sensor: %i", event->value);
					break;
				case REL_Z:
					ALOGE("BioHRM Sensor: %i", event->value);
					break;
				default:
					ALOGE("BioHRM Sensor Error: Default case. Code %i, Value %i", event->code, event->value);
					break;
				}
        } else if (type == EV_SYN) {
            mPendingEvent.timestamp = timevalToNano(event->time);
            if (mEnabled) {
                *data++ = mPendingEvent;
                count--;
                numEventReceived++;
            }
        } else {
            ALOGE("%s: unknown event (type=%d, code=%d)", LOG_TAG,
                    type, event->code);
        }

        mInputReader.next();
    }
    return numEventReceived;

}
