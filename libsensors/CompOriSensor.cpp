...
#define MAG_POLL_DELAY "/sys/devices/virtual/sensors/ssp_sensor/mag_poll_delay"
#define ROT_POLL_DELAY "/sys/devices/virtual/sensors/ssp_sensor/rot_poll_delay"
...
int CompOriSensor::setDelay(int32_t handle, int64_t ns)
{
    ...
    switch (handle) {
    case ID_M:
        ALOGE("Compass: delay=%lld ns", ns);
        strcpy(delay_path, MAG_POLL_DELAY);
        break;
    case ID_O:
        ALOGE("Orientation: delay=%lld ns", ns);
        strcpy(delay_path, ROT_POLL_DELAY);
        break;
    default:
        ALOGE("CompOriSensor: unknown handle %d", handle);
        return -EINVAL;
